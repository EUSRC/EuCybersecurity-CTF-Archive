				...
				success : function(data){
					...
					if(data.length>0){//没登录会返回首页html代码
						for(var i=0;i<data.length;i++){
							...
							str += '<li>';
							str += '<div class="shop_img">';
							str += '<img src="'+__IMG(data[i]["picture_info"]["pic_cover_big"])+'" alt="">';
							str += '</div>';
							str += '<a href="'+shop_main+'/goods/goodsinfo.html?goodsid='+data[i].goods_id+'" class="shop_name" title="'+data[i].goods_name+'&nbsp;'+data[i].sku_name+'">'+data[i].goods_name+'&nbsp;'+data[i].sku_name+'</a>';
							str += '<p class="price">￥'+data[i].price+'×'+data[i].num+'</p>';
							str += '<a href="javascript:;" class="delCartGoods" onclick="deleteShoppingCartById('+data[i].cart_id+')">删除</a>'
							str += '</li>';
						...
						$(".js-tip-box").hide();
						$('.js-footer-cart').show();
						$(".js-total").text("总价：￥"+total);
						$(".cart_info ul").html(str);
					}else{
						//再次查询
...
//				$.msg("请选择商品规格");
				$(".js-skulist").css("border","2px solid #000");
